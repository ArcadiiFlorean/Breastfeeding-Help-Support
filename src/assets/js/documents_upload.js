...
async function refreshDocumentsList() {
    try {
        ...
        const timestamp = new Date().getTime();
        const response = await fetch(`${DOCUMENTS_API}?refresh=${timestamp}&force=1`, {
            method: 'GET',
            headers: {
        ...
        if (data.success) {
            ...
            console.log('✅ Listă reîncărcată:', documentsData.length, 'documente active');
            
        } else {
            throw new Error(data.error || 'Eroare necunoscută la încărcarea documentelor');
        }
...
function displayDocumentsList() {
    ...
    documentsData.forEach((doc, index) => {
        ...
        const priceInfo = doc.is_free ? 
            ...
            `<span class="badge bg-warning text-dark">💰 ${doc.price || 0} RON</span>`;
        
        html += `
            <div class="document-item" data-doc-id="${doc.id}" style="animation-delay: ${index * 0.1}s">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    <div class="d-flex flex-wrap gap-2 align-items-center">
                        <span class="badge bg-primary">Document #${index + 1}</span>
                        ${priceInfo}
                        ${doc.is_featured ? '<span class="badge bg-warning text-dark">⭐ RECOMANDAT</span>' : ''}
                    </div>
                ...
                <div class="d-flex gap-3">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <p class="text-muted small mb-2">${escapeHtml(doc.description || 'Fără descriere')}</p>
                        <div class="d-flex flex-wrap gap-3 small text-muted">
                            <span><i class="fas fa-file me-1"></i>${escapeHtml(doc.original_filename)}</span>
                            <span><i class="fas fa-hdd me-1"></i>${formatFileSize(doc.file_size)}</span>
                            <span><i class="fas fa-file-alt me-1"></i>${getFileTypeDisplay(doc.file_type)}</span>
                            <span><i class="fas fa-calendar me-1"></i>${doc.created_at_formatted || new Date(doc.created_at).toLocaleDateString('ro-RO')}</span>
                            ${doc.downloads_count ? `<span><i class="fas fa-download me-1"></i>${doc.downloads_count} descărcări</span>` : ''}
                        </div>
                    </div>
                </div>
            </div>
...
async function deleteDocument(id) {
    if (!confirm('❌ Ștergi definitiv acest document?\n\nAcțiunea nu poate fi anulată!')) {
        return;
    }
...
function handleFileSelect(file) {
    console.log('📁 Fișier selectat:', file.name, '(', formatFileSize(file.size), ')');
    ...
    if (!allowedTypes.includes(file.type)) {
        showAlert('❌ Tipul de fișier nu este acceptat!\n\nAcceptate: PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX, TXT, imagini', 'danger');
        return;
    }
    
    // Validare dimensiune (10MB)
    if (file.size > 10 * 1024 * 1024) {
        showAlert('❌ Fișierul este prea mare!\n\nDimensiunea maximă permisă: 10MB', 'danger');
        return;
    }
...
async function uploadDocument(e) {
    ...
    const category = document.getElementById('document-category')?.value || 'general';
    const isFeatured = document.getElementById('document-featured')?.checked || false;
    const documentType = document.getElementById('document-type')?.value || 'free';
    ...
    if (documentType === 'paid' && price <= 0) {
        showAlert('❌ Pentru documentele cu plată, prețul trebuie să fie mai mare decât 0!', 'danger');
        return;
    }
    ...
    console.log('📋 Date upload:', {
        title, description, category, price, isFree, isFeatured,
    ...
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Se publică...';
    ...
    try {
        ...
        showAlert('❌ Eroare la încărcarea documentului: ' + error.message, 'danger');
    } finally {
        // Restaurează butonul
...
function getFileTypeDisplay(fileType) {
    ...
    return typeMap[fileType] || fileType.split('/')[1]?.toUpperCase() || 'DOCUMENT';
}
...
function debugDocuments() {
    ...
    if (documentsData.length > 0) {
        ...
        documentsData.forEach((doc, index) => {
            console.log(`  ${index + 1}. ID: ${doc.id}, Titlu: "${doc.title}", Mărime: ${doc.formatted_size || formatFileSize(doc.file_size)}`);
        });
    }
    ...
    const elements = [
        'documents-count', 'upload-area', 'document-file', 
        'document-upload-form', 'documents-list'
    ];
...
function resetDocumentsManager() {
    ...
    const listEl = document.getElementById('documents-list');
    if (listEl) {
        listEl.innerHTML = '<div class="text-center p-4"><i class="fas fa-spinner fa-spin me-2"></i>Se resetează...</div>';
    }
...
if (typeof window.openDocumentsModal === 'undefined') {
    ...
    console.log('⚠️ openDocumentsModal nu există în dashboard.js, o definesc aici');
}
...
setTimeout(() => {
    ...
    if (missingElements.length > 0) {
        console.warn('⚠️ Elemente lipsă pentru funcționalitate completă:', missingElements);
